...
sap.ui.define([
	...
	FHIRUtils.filterBuilder = function(aFilters, mParameters, iSupportedFilterDepth, bIsValueSet, iLvl, bLogicalConnection, bLogicalOperator){
		...
		if (aFilters){
			for (var i = 0; i < aFilters.length; i++) {
				...
				if (oFilter && oFilter._bMultiFilter && iLvl <= iSupportedFilterDepth){
	...
	FHIRUtils._filterBuilder = function(oFilter, mParameters, bIsValueSet, bLogicalConnection, bLogicalOperator){
		...
		// mapping of data elements to search parameter code missing
		// for Resource parameters a hard coded mapping is possible
		switch(sPath) {
		case "id": 
			sPath = "_id"; 
			break;
		...
		case "meta/profile/0":
			sPath = "_profile"; 
			break;
		case "meta/security/0":
			sPath = "_security"; 
			break;
		default:
			// map the binding path to a FHIRPath:
			// remove all array indexes and replace slashes by dots
/*			let aPathSegments = sPath.split("/");
			sPath = "";
			for (let sSegment of aPathSegements) {
				if (isNan(sSegment)) {
					sPath = sPath == "" ? sSegment : "." + sSegment;  
				}
			}
	*/		
		} 
		// for resource type specific + parameters pointing to nested elements the according Search Parameter
		...
		if (bIsValueSet) {
			...
			mParameters[sPath + sFhirSearchModifier] = ["ge" + oValue1, "le" + oValue2, "bt"];
