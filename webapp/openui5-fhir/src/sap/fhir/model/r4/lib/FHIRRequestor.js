...
sap.ui.define([
	...
	FHIRRequestor.prototype._buildQueryParameters = function(mParameters, oBindingInfo, sMethod) {
		...
		// D028776:
		// If a query parameter has several values they "normally" need to be combined by a OR:
		// This is true for all search parameters and the result set parameters _sort and elements.
		// In FHIR search this expressed via a comma-separated list: Patient?family=Miller,Meyer
		// The parameters _count, _summary, _total are by nature single valued.
		// The only exception are the parameters _include and _revinclude. They must not be multi-valued
		// but can be used severeal times per request e.g. 
		// Observation?_include=Observation:subject:Patient&_include=Observation:encounter:Encounter
		
		each(mParameters, function(sKey, vValue) {
			if (vValue && Array.isArray(vValue)) {
				if (sKey == "_include" || sKey == "_revinclude") {
					vValue.forEach(function(sItem) {
						aQuery.push(this._encodePair(sKey, sItem));
					}.bind(this));
				} else if (vValue.length == 3 && vValue[2] == "bt") {
					aQuery.push(this._encodePair(sKey, vValue[0]));
					aQuery.push(this._encodePair(sKey, vValue[1]));
				} else {
					var sParameter = this._encode(sKey, true) + "=";
					vValue.forEach(function(sItem, idx) {
						sParameter += (idx != 0) ? "," + this._encode(sItem, false) : this._encode(sItem, false);
					}.bind(this));
					aQuery.push(sParameter);
				}
			} else if (vValue) {
